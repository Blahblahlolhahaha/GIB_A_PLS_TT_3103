...
class Sender:
    def __init__(self, num_seconds, reliability_probability) -> None:
    ...
    def start(self):
        ...
        try:
            ...
            self._init_arrays()
            while time.time() - start < self.num_seconds:
                self.gamenet.start()
                for i in range(self.oneshot):
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        print("[USAGE] python3 sender.py <time> <reliability_probability>")
    else:
        num_seconds = int(sys.argv[1])
        reliability_probability = float(sys.argv[2])
        sender = Sender(num_seconds, reliability_probability)
        sender.start()
